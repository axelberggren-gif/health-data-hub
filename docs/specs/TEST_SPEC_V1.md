...
| **M4-T03** | **Benjamini–Hochberg unit test.** For p-values `[0.001, 0.008, 0.026, 0.028, 0.030, 0.06, 0.3, 0.9]` at q = 0.05, exactly the first **5** survive (BH step-up: largest k with p(k) ≤ k·q/m is k=5, since 0.030 ≤ 5·0.05/8 = 0.03125 — note this *rescues* k=3 and k=4, which fail their own thresholds; the helper must implement the step-up rule, not a per-p cutoff). Pure-function test on the FDR helper. |
